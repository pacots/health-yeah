...
export default function EmergencySummaryPage() {
  // Track if component has mounted on CLIENT (not during SSR)
  const [isMounted, setIsMounted] = useState(false);

  // Read context ONLY during actual renders (will be null until component mounts)
  const context = isMounted ? useApp() : null;
  ...
  // Set mounted flag ONLY on client after hydration
  useEffect(() => {
    setIsMounted(true);
  }, []);

  // SSR-safe: return nothing until client mounts (hydration succeeds, then renders)
  if (!isMounted) {
    return null;
  }

  // Now we're on client after hydration, safe to use context
